	...
	<div class="container">
		<div class="content">
			...
			<form class="form-inline" method="get">
				...
				<a href="input_nilai.php" data-toggle="tooltip" title="Tambah Nilai" class="btn btn-success" role="button"><span class="glyphicon glyphicon-plus"></span> Tambah Nilai </a>
			...
			<div class="table-responsive">
				<table class="table table-striped table-hover">
					...
					<?php
					$username = $_SESSION['id_dosen'];
					if($filter){
						$sql = mysqli_query($koneksi, "SELECT * FROM detail_matkul
							INNER JOIN tbl_mahasiswa ON detail_matkul.nim = tbl_mahasiswa.nim
							INNER JOIN dosen ON detail_matkul.dosen = dosen.nama_dosen
						WHERE grade='$filter' AND id_dosen='$username'"); 
					}else{
						$sql = mysqli_query($koneksi, "SELECT * FROM detail_matkul
								INNER JOIN tbl_mahasiswa ON detail_matkul.nim = tbl_mahasiswa.nim
								INNER JOIN dosen ON detail_matkul.dosen = dosen.nama_dosen WHERE id_dosen='$username'
								"); 
					}
						...
						while($row = mysqli_fetch_assoc($sql)){ 
							...
							echo '
								...
								<td>
									...
									<a href="edit_nilai.php?id_nilai='.$row['id_nilai'].'" title="Edit Data" data-toggle="tooltip" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
